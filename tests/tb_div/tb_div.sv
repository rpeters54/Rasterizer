...
logic                   rst_n_i;
logic                   vld_i;
logic [`DATA_WIDTH-1:0] numer_i;
logic [`DATA_WIDTH-1:0] denom_i;

logic                   rdy_in_o;
logic                   vld_o;
logic [`DATA_WIDTH-1:0] quot_o;

lp_div #(.DATA_WIDTH(`DATA_WIDTH), .FRAC_BITS(`FRAC_BITS), .SIGNED(0)) div_unit (
    .clk_i(clk_i),
    .rst_n_i(rst_n_i),
    .vld_i(vld_i),
    .numer_i(numer_i),
...
task test_div();
    ...
    expected = fixed_point_div(numer_i, denom_i);
...
// emulate fixed point division
function [`DATA_WIDTH-1:0] fixed_point_div(
...
shifted_numer  = {numer_i, {`FRAC_BITS{1'b0}}};
shifted_denom  = {{`FRAC_BITS{1'b0}}, denom_i};
shifted_result = shifted_numer / shifted_denom;

return shifted_result[`DATA_WIDTH-1:0];
