...
module tile_processor(
    ...
    output logic signed [`FX_TOTAL_BITS-1:0]     dzdx_o,
    output logic signed [`FX_TOTAL_BITS-1:0]     dzdy_o,
    output logic signed [`FX_TOTAL_BITS-1:0]     z_current_o
);
...
logic signed [`FX_TOTAL_BITS-1:0]   z_current;
        ...
        case (present_state)
            ...
            COMPUTE_PARTIALS : begin
                ...
                if (next_state == FORWARDNG_3) begin
                    dzdx <= div_result_dzdx[(`FX_TOTAL_BITS-1+`FX_FRAC_BITS):`FX_FRAC_BITS];
                    dzdy <= div_result_dzdy[(`FX_TOTAL_BITS-1+`FX_FRAC_BITS):`FX_FRAC_BITS];
                end
            end
            FORWARDNG_3 : begin
            ...
            COMPUTE_Z: begin
                ...
                if (next_state == PASS_ONWARD) begin
                    z_current <= z_temp[(`FX_TOTAL_BITS-1+`FX_FRAC_BITS):`FX_FRAC_BITS];
                end
            end
            PASS_ONWARD: begin
